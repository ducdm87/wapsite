...
class HomeController extends BackEndController {
    ...
    public function actionDisplay() {        
        ...
        $this->addIconToolbar("Creat", Router::buildLink("category", array("layout"=>"edit")), "new");
        $this->addIconToolbar("Edit", Router::buildLink("category", array("layout"=>"edit")), "edit", 1, 1, "Please select a item from the list to edit");        
        $this->addIconToolbar("Publish", Router::buildLink("category", array("layout"=>"publish")), "publish");
        $this->addIconToolbar("Unpublish", Router::buildLink("category", array("layout"=>"unpublish")), "unpublish");
        $this->addIconToolbar("Delete", Router::buildLink("category", array("layout"=>"remove")), "trash", 1, 1, "Please select a item from the list to Remove");        
    ...
    public function actionEdit() {
        ...
        $this->addIconToolbar("Save", Router::buildLink("category", array("layout"=>"save")), "save");
        $this->addIconToolbar("Apply", Router::buildLink("category", array("layout"=>"apply")), "apply");
        
        if ($cid == 0) {
            $this->addBarTitle("Category <small>[New]</small>", "user");        
            $this->pageTitle = "New menu item";
            $this->addIconToolbar("Cancel", Router::buildLink("category", array("layout"=>"cancel")), "cancel");        
        }else{
            $this->addBarTitle("Category <small>[Edit]</small>", "user");        
            $this->pageTitle = "Edit menu item";
            $this->addIconToolbar("Close", Router::buildLink("category", array("layout"=>"cancel")), "cancel");
        }
       ...
       if(!$user->isSuperAdmin()){
           ...
           $this->redirect(Router::buildLink("category"));
       }
    ...
    function actionApply() {
        $cid = $this->store();
        $this->redirect(Router::buildLink("category", array("layout"=>"edit",'cid'=>$cid)));
    }
