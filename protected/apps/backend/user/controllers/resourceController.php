...
class ResourceController extends BackEndController {
    ...
    public function actionDisplay() {
        ...
        if (!$user->isSuperAdmin()) {
            YiiMessage::raseNotice("[System] Your account not have permission to visit page");
            $this->redirect(Router::buildLink("cpanel"));
        }
         $this->addIconToolbar("New", Router::buildLink("user",array('view'=>'resource','layout'=>'new')), "new");
        $this->addIconToolbar("Edit", Router::buildLink("user",array('view'=>'resource','layout'=>'edit')), "edit", 1, 1, "Please select a item from the list to edit");        
        $this->addIconToolbar("Publish", Router::buildLink("user",array('view'=>'resource','layout'=>'publish')), "publish");
        $this->addIconToolbar("Unpublish", Router::buildLink("user",array('view'=>'resource','layout'=>'unpublish')), "unpublish");
        $this->addIconToolbar("Delete", Router::buildLink("user",array('view'=>'resource','layout'=>'remove')), "trash", 1, 1, "Please select a item from the list to Remove");        
    ...
    public function actionEdit($type = false) {
        ...
        $this->addIconToolbar("Save", Router::buildLink("user",array('view'=>'resource','layout'=>'save')), "save");
        $this->addIconToolbar("Apply", Router::buildLink("user",array('view'=>'resource','layout'=>'apply')), "apply");
        $this->addBarTitle("Permission <small>[Edit]</small>", "user");
        $this->addIconToolbar("Close", Router::buildLink("user",array('view'=>'resource','layout'=>'cancel')), "cancel");
    ...
    function actionApply() {
        ...
        $this->redirect(Router::buildLink("user",array('view'=>'resource','layout'=>'edit','cid'=>$cid)));
    }
    
    function actionSave() {
        $cid = $this->store();
        YiiMessage::raseSuccess("Successfully save Permission");
        $this->redirect(Router::buildLink("user",array('view'=>'resource')));
    }
