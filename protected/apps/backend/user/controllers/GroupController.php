...
class GroupController extends BackEndController {
    ...
    public function actionDisplay() {
        ...
        $this->addIconToolbar("Edit", Router::buildLink("user", array("view" => "group", "layout" => "edit")), "edit", 1, 1, "Please select a item from the list to edit");
        $this->addIconToolbar("New", Router::buildLink("user", array("view" => "group", "layout" => "new")), "new");
//        $this->addIconToolbarDelete();
        $this->addIconToolbar("Delete", Router::buildLink("user", array("view" => "group", "layout" => "remove")), "trash", 1, 1, "Please select a item from the list to Remove");
    ...
    function changeStatus($cid, $value) {
        ...
        if ($bool == false) {
            ...
            $this->redirect(Router::buildLink("user", array('view' => 'group')));
            return false;
        }
    ...
    public function actionEdit() {
        ...
        $this->addIconToolbar("Save", Router::buildLink("user", array("view" => "group", "layout" => "save")), "save");
        $this->addIconToolbar("Apply", Router::buildLink("user", array("view" => "group", "layout" => "apply")), "apply");
        $items = array();
        ...
        if ($cid == 0) {
            $this->addIconToolbar("Cancel", Router::buildLink("user", array("view" => "group", "layout" => "cancel")), "cancel");
            $this->addBarTitle("User group <small>[New]</small>", "user");
            $this->pageTitle = "New group";
        } else {
            $this->addIconToolbar("Close", Router::buildLink("user", array("view" => "group", "layout" => "cancel")), "cancel");
    ...
    function actionApply() {
        ...
        $this->redirect(Router::buildLink("user", array("view" => "group", "layout" => "edit", 'cid' => $cid)));
    }
